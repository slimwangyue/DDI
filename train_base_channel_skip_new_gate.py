...
def parse_args():
    ...
    parser.add_argument('--branch', default='False',
                        help='Whether overall early exit effect is tested')
...
def main():
    args = parse_args()
    args.save_path = save_path = os.path.join(args.save_folder, args.arch + '_skip_channel_new_gate_minimum_' + str(args.minimum) + '_Early_Exit_' + 'beta_' + str(args.beta) + args.loss)
    os.makedirs(args.save_path, exist_ok=True)
    
    if args.branch == 'True':
        args.logger_file = os.path.join(save_path, 'log_{}_branch.txt'.format(args.cmd))
    else:
...
def train(args, train_loader, model, criterion, train_criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # compute output
        output,masks,gprobs,output_branch = model(input_var)
        ...
        if args.computation_cost:
            ...
            for idx in range(len(output_branch)):
                loss = loss + train_criterion(output_branch[idx], target_var) 
        ...
        for idx in range(len(output_branch)):
            ...
            top1_list[idx].update(prec1_branch[0], input.size(0))
        ...
        if i % args.print_freq == 0:
            logging.info(
                ...
                "Prec_B7@1 {top1_b7.val:.3f}({top1_b7.avg:.3f})\t"
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        if i % args.print_freq == 0:
            logging.info(
                ...
                "Prec_B5@1 {top1_b5.val:.3f} ({top1_b5.avg:.3f})\t"
                "Prec_B6@1 {top1_b6.val:.3f} ({top1_b6.avg:.3f})\t"
                "Prec_B7@1 {top1_b6.val:.3f} ({top1_b7.avg:.3f})\t"
                'Prec_Main@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                'Prec_Main@5 {top5.val:.3f} ({top5.avg:.3f})\t'
                'Energy_ratio: {cp_record.val:.3f}({cp_record.avg:.3f})\t'.format(
                    i, 
                    ...
                    top1=top1, 
                    top5=top5, 
                    cp_record = cp_record, )) # cp_record=cp_record))
